...
{
	//Allow users to access the next and previous nodes without 
    public static class ReadOnlyNode<E>
    {
        /**
         ...
         */
        public ReadOnlyNode(E e, Node<E> p, Node<E> n)
        {
            element = e;
            prev = p;
            next = n;
     ...
     */
    private static class Node<E> extends ReadOnlyNode<E>
    {
    ...
    public ReadOnlyNode<E> getFirstRONode()
    {
        if (isEmpty())
        {
            return null;
    ...
    public ReadOnlyNode<E> getNext( ReadOnlyNode<E> node )
    {
        if( node instanceof Node )
        {
            Node<E> next = ((Node) node).getNext();
            return ( next == trailer )?null:next;
        }
        throw new RuntimeException("ERROR: Cannot deal with something other than my Node" );
    }

    public ReadOnlyNode<E> getPrev( ReadOnlyNode<E> node )
    {
        if( node instanceof Node )
        {
            Node<E> ret = ((Node) node).getPrev();
            return ( ret == header )?null:ret;
        }
        throw new RuntimeException("ERROR: Cannot deal with something other than my Node" );
    }
